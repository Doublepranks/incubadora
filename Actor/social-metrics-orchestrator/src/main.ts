...
function buildActorInput(platform: string, profiles: ProfileInput[]): Record<string, unknown> {
    ...
    switch (platform) {
        ...
        case 'youtube':
            ...
            return {
                startUrls: usernames.map(u => ({ url: `https://www.youtube.com/@${u}` })),
                maxResults: 1,
            };
    ...
    try {
        ...
        switch (platform) {
            ...
            case 'x':
                ...
                username = String(item.userName || item.screen_name || '');
                followers = Number(item.followers || item.followersCount || 0);
                posts = Number(item.statusesCount || item.tweetsCount || 0);
                break;
        ...
        if (!username) {
            console.log(`⚠️ Could not extract username from ${platform} result`);
            return null;
        }
