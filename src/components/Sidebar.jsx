...
const Sidebar = () => {
    ...
    // Extract unique states and municipalities
    const states = ['Todos', ...new Set(allInfluencers.map(inf => inf.state))];
    const municipalities = ['Todos', ...new Set(
        allInfluencers
            .filter(inf => selectedState === 'Todos' || inf.state === selectedState)
            .map(inf => inf.municipality)
    )];
        ...
        <div className="w-64 h-screen bg-white dark:bg-gray-900 border-r border-gray-200 dark:border-gray-800 flex flex-col fixed left-0 top-0 transition-colors duration-300">
            ...
            <nav className="flex-1 overflow-y-auto p-4 space-y-6">
                ...
                <div>
                    ...
                    <div className="space-y-1">
                        <h3 className="text-xs font-semibold text-gray-500 mb-2">Resultados ({filteredInfluencers.length})</h3>
                        {filteredInfluencers.map(inf => (
                            ...
                            >
                                ...
                                <div className="truncate">
                                    ...
                                    <div className="text-xs text-gray-500 truncate">{inf.municipality} - {inf.state}</div>
                                </div>
                            </button>
                        ))}
                    </div>
