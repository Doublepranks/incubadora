...
const Sidebar = () => {
    ...
    const LinkItem = ({ to, icon: Icon, label }) => {
        ...
        return (
            ...
            >
                <Icon size={18} className={`${isSidebarCollapsed ? '' : 'mr-3'} transition-transform duration-300 ${active ? "scale-110" : "group-hover:scale-110"}`} />
                {!isSidebarCollapsed && <span className="relative z-10">{label}</span>}
        ...
        <div className={`fixed left-4 top-4 bottom-4 ${isSidebarCollapsed ? 'w-20' : 'w-64 xl:w-72'} glass-panel rounded-3xl flex flex-col z-[999] overflow-visible border border-white/5 bg-zinc-950/80 backdrop-blur-xl transition-all duration-300 ease-in-out`}>
            ...
            <nav className="flex-1 overflow-y-auto p-4 space-y-8 custom-scrollbar">
                ...
                {(user?.role === 'system_admin' || user?.role === 'admin_global') && (
                    <div className="space-y-1">
                        {!isSidebarCollapsed && <h2 className="px-4 text-[10px] font-bold text-zinc-500 uppercase tracking-widest mb-2 font-mono whitespace-nowrap overflow-hidden">Admin</h2>}
                        {user?.role === 'system_admin' && <LinkItem to="/sysadmin" icon={ServerCog} label="Sistema" />}
                        <LinkItem to="/users" icon={Shield} label="Usuários" />
                    </div>
                )}

                {/* Filters / Explore Section - Hidden when collapsed */}
                {!isSidebarCollapsed && (
                ...
                {/* Live Results List - Hidden when collapsed */}
                {!isSidebarCollapsed && sidebarResults.length > 0 && (
            ...
            <div className={`p-4 border-t border-white/5 bg-zinc-900/30 flex flex-col gap-3 ${isSidebarCollapsed ? 'items-center' : ''}`}>

                {/* Toggle Button moved to footer */}
                <button
                    onClick={toggleSidebar}
                    className={`flex items-center justify-center ${isSidebarCollapsed ? 'h-8 w-8 rounded-full' : 'w-full py-2 rounded-lg text-xs font-medium'} bg-zinc-800/50 hover:bg-zinc-700 hover:text-white text-zinc-400 border border-white/5 transition-colors`}
                ...
                >
                    {isSidebarCollapsed ? (
                        ...
                        <>
                            <PanelLeftClose size={14} className="mr-2" />
                    ...
                    !isSidebarCollapsed ? (
                        <>
                            ...
                            <button
                                ...
                                className="w-full flex items-center justify-center py-2 px-4 rounded-lg bg-red-500/10 hover:bg-red-500/20 text-red-400 hover:text-red-300 text-xs font-medium transition-all border border-red-500/20"
                            >
                        ...
                        <>
                            <div className="h-9 w-9 rounded-full bg-gradient-to-tr from-zinc-700 to-zinc-600 border border-white/10 flex items-center justify-center" title={user.name}>
                                <span className="font-semibold text-white/80 text-xs">{user.name.charAt(0).toUpperCase()}</span>
                            </div>
                            <button
                                onClick={() => logout().then(() => navigate("/login"))}
                                className="h-8 w-8 flex items-center justify-center rounded-lg bg-red-500/10 hover:bg-red-500/20 text-red-400 hover:text-red-300 transition-all border border-red-500/20"
